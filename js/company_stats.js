...
function buildDailyTripTrend(trips, rangeStart) {
  const counts = countBy(
    trips.filter((trip) => trip.start_at),
    (trip) => formatDayKey(trip.start_at)
  );
...
export async function loadCompanyStats(companyId, range = "30d") {
  ...
  return {
    ...
    charts: {
      dailyTrips: buildDailyTripTrend(tripsInRange, rangeStart)
    },
    incidents: {
